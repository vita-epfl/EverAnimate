...
mkdir -p "${OUTPUT_PATH:-experiments/720p}" logs
...
accelerate launch --num_processes "${NUM_PROCESSES:-1}" scripts/everanimate_train.py \
  ...
  --output_path "${OUTPUT_PATH:-experiments/720p}" \
  --lora_base_model "dit" \
  ...
  --same_augmentation \
  --lora_checkpoint "${LORA_CHECKPOINT:-ckpts/everanimate-v1-lora32/stage2_480p.safetensors}" \
