...
class WanAnimateAdapter(torch.nn.Module):
    ...
    def after_patch_embedding(self, x: List[torch.Tensor], pose_latents, face_pixel_values, num_video_anchor_latents=1):
        pose_latents = self.pose_patch_embedding(pose_latents)
        x[:, :, num_video_anchor_latents:] += pose_latents
        ...
        pad_face = torch.zeros(B, num_video_anchor_latents, H, C).type_as(motion_vec)
