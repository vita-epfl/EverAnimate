...
class WanVideoUnit_ImageEmbedderVAE(PipelineUnit):
    ...
    def process(self, pipe: WanVideoSviProPipeline, input_image, end_image, num_frames, height, width, tiled, tile_size, tile_stride, anchor=None, inference_mode=False, prev_last_latent=None, auxiliary_video=None, num_motion_latent=2):
        ...
        if is_first_clip:
            ...
            if getattr(pipe, "pad_first_clip_with_anchor", False):
                motion_latent = anchor_latent.repeat(1, num_motion_latent, 1, 1)
            else:
                motion_latent = anchor_latent.new_zeros(
                    anchor_latent.shape[0], num_motion_latent, anchor_latent.shape[2], anchor_latent.shape[3],
                )
            padding_size = total_latents - anchor_latent.shape[1] - num_motion_latent
            padding = torch.zeros(
