        ...
        if self.require_downloading():
            ...
            if self.origin_file_pattern is None or self.origin_file_pattern == "":
                ...
                if isinstance(self.path, list) and len(self.path) == 0:
                    raise FileNotFoundError(f"No files found matching pattern '{self.origin_file_pattern}' in {os.path.join(self.local_model_path, self.model_id)}. Download may have failed or pattern is incorrect.")
        elif self.path is None and self.model_id is not None:
            # When skip_download is True, but path is not set, construct the local path
            if self.origin_file_pattern is None or self.origin_file_pattern == "":
                self.path = os.path.join(self.local_model_path, self.model_id)
            else:
                self.path = glob.glob(os.path.join(self.local_model_path, self.model_id, self.origin_file_pattern))
                if isinstance(self.path, list) and len(self.path) == 0:
                    raise FileNotFoundError(f"No files found matching pattern '{self.origin_file_pattern}' in {os.path.join(self.local_model_path, self.model_id)}. Model files don't exist locally. Either download first or set DIFFSYNTH_SKIP_DOWNLOAD=False.")
